 ...
 * MIH 01: v5.2.2 has a bug when select file: slow processing animation
 * MIH 02: v5.1.5 has a bug when delete too large file: message componet typographic error
 * MIH 03: v5.0.7 has a bug after upload duplicate file: missing preview thumbnail
...
(function (factory) {
  ...
  FileInput.prototype = {
    ...
    _initFileManager: function () {
      ...
      self.fileManager = {
        ...
        isPending: function (id) {
          return (
            $.inArray(id, self.fileManager.processed) === -1 &&
    ...
    _initResumableUpload: function () {
      ...
      self.resumableManager = {
        ...
        loopAjax: function () {
          var rm = self.resumableManager;
          if (
            rm.currThreads < self.resumableUploadOptions.maxThreads &&
            !rm.testing
          ) {
    ...
    _initTemplateDefaults: function () {
      ...
      tPreview =
        ...
        "  </div>\n" +
        "</div>";
      // MIH Fix 04-1: Use icon replace characters
      tClose = $h.closeButton("fileinput-remove bi-x");
      tFileIcon = '<i class="bi-file-earmark-arrow-up"></i>'; //MIH Fix 12: Use bootstrap icon instead of glyphicon, match "glyphicon-" 50 times.
    ...
    /* MIH 10-2 */
    _beforeAjaxEnqueue: function (fileId, settings) {
      var self = this;
      if (!self.beforeAjaxEnqueue) return settings;
      return self.beforeAjaxEnqueue(fileId, settings);
    },
    _ajaxSubmit: function (
      ...
      // MIH 09: Remove useless params from formdata
      ...
      /* MIH 10-1: Add handler to modify Ajax params before request */
      settings = self._beforeAjaxEnqueue(fileId, settings);
    ...
    // MIH 08: Add function to delete normal display but duplicate(MD5) file
    deleteFileViaThumb: function (frame) {
      var self = this;
      $h.cleanMemory(frame),
        (id = $frame.attr("id")),
        (ind = $frame.attr("data-fileindex"));
    ...
    _previewFile: function (i, file, theFile, data, fileInfo) {
      ...
      // MIH Fix 05: readFiles not clear background text when clipboard image pasted
      self.$container
    ...
    // MIH 11: Add function to custom progress bar text
    setCustomProgressTip(p, tip) {
      var self = this,
        defaultTip = self.msgUploadThreshold,
        p = Math.min(100, Math.max(p, self.progressUploadThreshold));
      self.msgUploadThreshold = tip;
      self._setProgress(p);
      self.msgUploadThreshold = defaultTip;
    },
    ...
    _setFileDropZoneTitle: function () {
      ...
      if (
        ...
        // MIH Fix 06: Let DropZoneTitle cannot selected
        $zone.prepend(
    ...
    customMsgs: function (errorMsgs) {
      // MIH Fix 07: Add post custom message method
      var self = this,
        $error = self.$errorContainer;
      errorMsgs = "<li>" + errorMsgs.join("</li><li>") + "</li>";
      if ($error.find("ul").length === 0) {
        $error.html(self.errorCloseButton + "<ul>" + errorMsgs + "</ul>");
      } else {
        $error.find("ul").append(errorMsgs);
      }
      $error.fadeIn(self.fadeDelay);
      self._handler($error.find(".kv-error-close"), "click", function () {
  ...
  $.fn.fileinput.defaults = {
    ...
    textEncoding: "UTF-8",
    ajaxSettings: {},
    beforeAjaxEnqueue: null, // MIH 10-3 function (ajaxSettings){}
    ajaxDeleteSettings: {},
